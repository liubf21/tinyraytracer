...
struct PPMnode // photon map node (actually store the intersection point of the ray and the object)
{
    ...
    PPMnode(Point3 pos_, Vec3f col_, Vec3f dir_, int index_ = -1, float prob_ = 1, float r_ = 1):
...
public:
    ...
    ~KDTree() { if (tree != nullptr) delete[] tree; }
    ...
    int bt(int l, int r, int d)
    {
        ...
        if (l < mid) tree[mid].s[0] = bt(l, mid - 1, (d + 1) % 3), mt(mid, tree[mid].s[0]); // left
        if (mid < r) tree[mid].s[1] = bt(mid + 1, r, (d + 1) % 3), mt(mid, tree[mid].s[1]); // right
        return mid;
    }
    void init(std::vector<PPMnode> &pm) // use PPMnode to initialize the tree
    {
        ...
        for (int i = 0; i < n; ++i) tree[i].pm = pm[i]; // copy
    ...
    void _query(const PPMnode &node, IMGbuf* c, int o) // query whether the node is in the box of o, if so, add to the buffer
    {
        ...
        float dis2[2];
        if (tree[o].s[0] != -1) dis2[0] = getdis2(node.pos, tree[tree[o].s[0]].m[0], tree[tree[o].s[0]].m[1]);
        else dis2[0] = max_float;
        if (tree[o].s[1] != -1) dis2[1] = getdis2(node.pos, tree[tree[o].s[1]].m[0], tree[tree[o].s[1]].m[1]);
        else dis2[1] = max_float;
        int tmp = dis2[0] >= dis2[1];
        if (dis2[tmp] < epsilon) _query(node, c, tree[o].s[tmp]); // if in the box, recursively query
        tmp ^= 1;
        if (dis2[tmp] < epsilon) _query(node, c, tree[o].s[tmp]);
    }
    void _modify(int o) // modify the min and max of o, used when the radius of o is changed
    {
        ...
        if (tree[o].s[0] != -1) _modify(tree[o].s[0]), mt(o, tree[o].s[0]);
        if (tree[o].s[1] != -1) _modify(tree[o].s[1]), mt(o, tree[o].s[1]);
    }
